...
def ex1(k: int, attr: 'np.ndarray[np.float32]') -> 'np.ndarray[np.float32]':
    ...
    """
    indices = np.argsort(attr)
    result = np.zeros(attr.shape)

    n_clusters = len(attr) // k

    for i in range(n_clusters):
        j = indices[k * i:] if i == n_clusters - 1 else indices[k * i: k * (i + 1)]

        result[j] = np.mean(attr[j])
...
def ex2(k: int, dataset: 'np.ndarray[np.float32]') -> 'np.ndarray[np.float32]':
    ...
    """
    attrs = np.copy(np.transpose(dataset))

    for i in range(len(attrs)):
        attrs[i] = ex1(k, attrs[i])
...
def plot_table(data: 'np.ndarray[np.float32]', row_labels: List[str], col_labels: List[str]):
    ...
    ax.axis('tight')
    ax.axis('off')
    table = ax.table(cellText=df.map('{:,.0f}'.format).values,rowLabels=row_labels,rowColours=colors,colLabels=df.columns,fontsize=50,loc="center")
...
if __name__ == '__main__':
    plot_table(ex2(3, ndata), row_labels, col_labels)