...
# def utility(data, adata):
# 	data, adata = np.reshape(data, -1), np.reshape(adata, -1)
# 	row_data = len(data)
# 	T = np.array([0 for _ in range(row_data)] + [1 for _ in range(row_data)])
# 	logit = sm.Logit(T, np.concatenate((data, adata)))
# 	result = logit.fit(method = 'bfgs', maxiter=1000)

# 	return np.mean((result.predict(adata) - 0.5) ** 2)
